...
// Mac OS: '\r', Linux: '\n', Windows: '\r\n'
std::istream&
ReadLine(std::istream& in, std::string& str)
{
    bool done = false;
    bool car_ret = false;
...
void
PrintAutoGenInfo(std::ofstream& f)
{
    f << "/*\n"
...
void
PrintStartNamespace(std::ofstream& f)
{
    f << "namespace ns3 {\n" << std::endl;
}

void
PrintEndNamespace(std::ofstream& f)
{
    f << "}\n" << std::endl;
}

void
PrintIncludes(std::ofstream& f, const std::string& type)
{
    if (type == "header")
    {
    }
...
void
PrintLeapSeconds(std::ofstream& f,
                 ...
                 const std::vector<uint32_t>& leap)
{
    ...
    else
    {
        f << "const uint32_t IersData::BaseLeapSeconds = 10;" << std::endl
          << "const uint32_t IersData::CurLeapSeconds = " << leap.size() + 10 << ";" << std::endl
...
void
PrintEopParams(std::ofstream& f, const EopParameters& eop)
{
    f << "{" << eop.xp << "," << eop.yp << "," << eop.dut1 << "," << eop.lod << "}";
}

void
PrintEopParameters(std::ofstream& f,
                   const std::string& type,
                   const std::string& visibility,
    ...
    if (type == "header")
    {
        if (visibility == "public")
            f << "  struct EopParameters {\n"
              ...
              << "    double xp, yp, dut1, lod;\n"
              << "  };\n\n"
              << "  static const std::vector<EopParameters> EopValues;" << std::endl;
        else
...
std::vector<uint32_t>
ReadLeapSeconds(std::ifstream& f)
{
...
std::vector<EopParameters>
ReadEopParameters(std::ifstream& f)
{
...
main(int argc, char* argv[])
{
    ...
    if (!leap.is_open())
    {
        ...
        std::cerr << base_path << "tai-utc.dat" << std::endl;
